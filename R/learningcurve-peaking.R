...
classifiers <- list(
  ...
  "EM" = function(X,y,X_u,y_u) {
    EMLeastSquaresClassifier(X,y,X_u, eps = 1e-4)
  },
  "SL" = function(X,y,X_u,y_u) {
    SelfLearning(X,y,X_u,method=LeastSquaresClassifier)
  },
  "Projection" = function(X,y,X_u,y_u) {
    ICLeastSquaresClassifier(X,y,X_u, projection="semisupervised")
  },
...
models <- models[c("Parkinsons","WDBC")]
datasets <- datasets[c("Parkinsons","WDBC")]

set.seed(42)
lc <- LearningCurveSSL(models,datasets,
                       classifiers=classifiers,
                       measures=measures,sizes=1:150,
                       n_l=10,repeats=200,verbose=TRUE,
